...
- Próxima tarea: **H7 — pulido (7 ítems)**. Automatizables por el loop: búsqueda/navegación RF-APP-005 (Should, candidata SPEC-018), aviso de consentimiento de grabación (Must, pequeña — puede ir con SPEC-018 o sola), degradación elegante sin diarización (Should), medición de coste IA por entrevista (Must), empaquetado/hardening final (Must). Manuales/humano: optimización de latencia (medir con sesión real), tests de usabilidad (Riesgo #2). Evaluar qué agrupar por spec al abrir la vuelta.
