...
- Última spec cerrada: **SPEC-013** (2026-07-04, unit 189/189 PASS a la primera; commit 81efb27; crear entrevista + detalle mínimo).
- Próxima tarea: **H3 ítems 2-5 en una sola spec (SPEC-014)** — generar guión personalizado con Claude (RF-GUION-002) + contexto histórico de la empresa (RF-GUION-003) + objetivos (RF-GUION-004) + edición (RF-GUION-005). Cierra H3. **NO hay ANTHROPIC_API_KEY aún** (ni en .env.local ni configurada): implementar con patrón de degradación (la UI de Ajustes ya soporta la clave Anthropic vía SPEC-007); la fase-0 de conexión real queda pendiente de que el humano ponga la key. Leer la skill claude-api antes de fijar el modelo en la spec/plan.
