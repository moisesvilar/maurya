...
- Próxima tarea: **H4 ítems 1-5 en una sola spec (SPEC-015)** — integrar la captura+transcripción del spike en el flujo real de la entrevista: sección Grabación en el detalle (iniciar/detener RF-AUDIO-001, captura RF-AUDIO-002, transcripción en vivo RF-AUDIO-003, permisos/dispositivos RF-AUDIO-005, atribución RF-AUDIO-004), persistiendo wavPath/transcriptPath en la Interview y status → 'recorded'. Reutiliza masivamente los componentes/hooks del spike (spike/*, useAudioCapture, useTranscription). El ítem 6 (sesión ≥60 min) es manual del humano. Decidir en la spec el destino de la página /capture (¿se mantiene como harness de pruebas o se deroga?).
