...
- Próxima tarea: **H3 — preparación con IA** (5 ítems). Ítem 1: RF-GUION-001 Must — crear entrevista dentro de empresa/contacto y asignar template. Los ítems 2-4 (guión LLM con Claude + contexto histórico + objetivos) necesitan una **ANTHROPIC_API_KEY** que el humano puede configurar ya en Ajustes (SPEC-007) o en .env.local — verificar antes de la spec de LLM. La entidad Interview (SPEC-006) ya existe con scriptMarkdown/objectives/status.
