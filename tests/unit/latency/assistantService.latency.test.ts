 ...
 * acotada por schema (maxLength 200/140) y max_tokens 4096 (512 originalmente;
 * subido porque el thinking adaptativo consume max_tokens y truncaba el JSON).
...
describe('assistantService (latencia SPEC-023)', () => {
  ...
  // SPEC-023 · AC-06 (valor actualizado: 512 → 4096, ver comentario de
  // MAX_TOKENS en assistantService.ts — el thinking adaptativo consume
  // max_tokens y con 512 truncaba el JSON de forma aleatoria)
  it('sends max_tokens 4096 for the assistant analysis calls', async () => {
    harness.create.mockResolvedValue(sdkResponse(analysisPayload()))
    startSession()
    feedFinal()
    feedFinal()
    ...
    expect(createCall(0).max_tokens).toBe(4096)
  })
})
